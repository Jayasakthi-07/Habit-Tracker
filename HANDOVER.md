...
| **7** | **AI coaching (Gemini + OpenAI)** | ✅ DONE (mobile) — built in `aura_core/lib/src/ai/` (one `AiCoach` interface, `GeminiCoach` + `OpenAiCoach` REST impls, `AiConfig` keys from dart-define, `AiCoachFactory`). Mobile **AI Coach** chat screen grounds prompts in real habit data; user-selectable provider when both keys present; gracefully gated when no key. **Verified LIVE** end-to-end on emulator: user added `OPENAI_API_KEY` to env.json; "Analyze my week" returned a real, data-grounded coaching reply (named the user's gym/water/sleep habits with correct stats). Gemini path is built+ready (add `GEMINI_API_KEY` to use/switch). Guide: `AI_SETUP.md`. |
| **8** | **Parity, light theme & polish** | ✅ DONE — **OpenRouter** added as first-priority AI provider (auto-fallback to OpenAI), prompt tuned for Aura + plain-text output. **Shared catalogs in aura_core** so both apps match: **74 achievements** (`achievements_catalog.dart`, metric-based) + one **habit-icon set** (`catalog/habit_icons.dart`, fixes Android wrong glyphs). **Light/Dark/System theme** on both apps (brightness-aware `AppColors` getters; `tool/fix_const.py` removed the now-invalid `const`s; toggle in Settings). **AI Coach added to Windows** (sidebar `/ai`, same shared coach). Mobile **smoothness**: list cards drop per-card BackdropFilter (`blur:0`) + no overscroll glow. Verified on emulator (light/dark, 74 achievements, smooth lists) and desktop builds clean. |
| **—** | **v1.0.0 GitHub release** | ✅ DONE — `gh release v1.0.0` on `feature/cloud-platform` with **AuraHabits-v1.0.0.apk** (Android, minSdk 23) + **AuraHabits-Windows-v1.0.0.zip**. 🔑 Built with `env.release.json` (Supabase-only safe defines) so **no AI/Google secrets are in the public artifacts**; users add their own AI key in-app. Release: github.com/Jayasakthi-07/Habit-Tracker/releases/tag/v1.0.0 |
| **—** | **v1.1.0 "Aurora" — full premium redesign + fixes** | ✅ DONE — complete UI redesign of BOTH apps ("Aurora": unified indigo `#6366F1`→violet `#8B5CF6` identity, ink-navy dark `#0B0E15` / porcelain light `#F6F7FB`, theme-aware shadows, aurora ambient blobs, floating glass bottom nav + gradient FAB on Android, grouped sidebar + gradient hero on Windows, premium `CompletionTick` coin with draw-on check + spring pop + burst ring). **Critical sync fix**: per-id push chaining + local-dirty echo guard in `SyncEngine` — rapid habit toggles no longer revert (the "needs multiple taps" bug). Mobile tabs lazy-load (Insights no longer recomputes per tap). **AI Coach now works in public releases**: runtime API keys via in-app key sheet/dialog (`AiConfig.runtime*` precede dart-define; stored only in local settings box) — release artifacts still contain NO secrets. Versions bumped to 1.1.0+2. |
| 9 | Gamification expansion (challenges/leaderboards) | ⬜ pending (74 achievements done) |
| 10 | Notifications & smart scheduling (FCM) | ⬜ pending |
| 11 | Branding & Play Store readiness | ⬜ pending (release SHA-1 + real keystore for Play) |
