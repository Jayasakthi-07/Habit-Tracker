...
/// license keys). Ported verbatim from the Windows app so behaviour matches.
class UserProfile {
  const UserProfile({
    required this.name,
    this.email = '',
  ...
  UserProfile copyWith({String? name, String? email, bool? isPremium}) =>
      UserProfile(
        name: name ?? this.name,
...
class AuthController extends Notifier<UserProfile?> {
  ...
  bool _premiumLocal() =>
      HiveService.dynBox(Boxes.profile).get(_kPremium, defaultValue: false)
          as bool;
  ...
  bool get _rememberFlag =>
      HiveService.dynBox(Boxes.profile).get(_kRemember, defaultValue: false)
          as bool;
  ...
  /// - `error != null` → failed.
  /// - `needsVerification == true` → a verification code was emailed; call
  ...
  Future<({String? error, bool needsVerification})> signUpWithEmail({
    ...
    try {
      final res = await _auth!
          .signUpWithEmail(email: email, password: password, fullName: name);
  ...
  // ---- Google (native google_sign_in id_token -> Supabase) ----
...
final authProvider =
    NotifierProvider<AuthController, UserProfile?>(AuthController.new);
